...
// SysProcIDMap holds Container ID to Host ID mappings used for User Namespaces in NSX.
...
type SysProcAttr struct {
	...
	Ctty        int            // Controlling TTY fd (Linux only, also NSX?)
	Pdeathsig   Signal         // Signal that the process will get when its parent dies (Linux only, also NSX?)
	Cloneflags  uintptr        // Flags for clone calls (Linux only, also NSX?)
...
func forkAndExecInChild(argv0 *byte, argv, envv []*byte, chroot, dir *byte, attr *ProcAttr, sys *SysProcAttr, pipe int) (pid int, err Errno) {
	...
	for i = 0; i < len(fd); i++ {
		...
		if fd[i] == int(i) {
			// dup2(i, i) won't clear close-on-exec flag on Linux, (what about NSX?)
