...
func UtimesNano(path string, ts []Timespec) (err error) {
	...
	// in 2.6.22, Released, 8 July 2007) then fall back to utimes - What about NSX?
	var tv [2]Timeval
	for i := 0; i < 2; i++ {
		tv[i].Sec = ts[i].Sec
...
func Futimes(fd int, tv []Timeval) (err error) {
	...
	// (and is what glibc does). Confirm the same case for NSX?
...
func Getgroups() (gids []int, err error) {
	...
	// Sanity check group count.  Max is 1<<16 on Linux. What about NSX?
	if n < 0 || n > 1<<20 {
		return nil, EINVAL
	}
...
func anyToSockaddr(rsa *RawSockaddrAny) (Sockaddr, error) {
	...
	case AF_UNIX:
		...
		// This is not technically the Linux/NSX? semantics for
...
func Reboot(cmd int) (err error) {
	return reboot(NSX_REBOOT_MAGIC1, NSX_REBOOT_MAGIC2, cmd, "")
}
...
// mmap varies by architecture; see syscall_nsx_*.go.
