...
// Copying procedures while porting to NSX
...
static version_key ns26 = { (byte*)"NSX", 0x3ae75f6 };
...
static void
runtime·nsx_setup_vdso(int32 argc, uint8** argv)
{
	...
	for(int32 i=0; elf_auxv[i].a_type!=AT_NULL; i++) {
		if(elf_auxv[i].a_type == AT_SYSINFO_EHDR) {
			...
			vdso_parse_symbols(&vdso_info, vdso_find_version(&vdso_info, &nsx26));
			continue;
		}
...
void (*runtime·sysargs)(int32, uint8**) = runtime·nsx_setup_vdso;
