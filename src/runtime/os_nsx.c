...
#include "os_GOOS.h"
#include "signal_unix.h" 									// May need to change for NSX (?)
#include "stack.h"
#include "textflag.h"
...
// NSX futex. (Does this exist for NSX? Copied from Linux procedures)
//
...
runtime·futexsleep(uint32 *addr, uint32 val, int64 ns)
{
	...
	// Assuming same scenario for NSX 
...
// from the ELF AT_RANDOM auxiliary vector (vdso_nsx_amd64.c).
...
{
	mp->gsignal = runtime·malg(32*1024);	// OS X wants >=8K, NSX >=2K (are we sure?)
	mp->gsignal->m = mp;
}
...
runtime·setsig(int32 i, GoSighandler *fn, bool restart)
{
	...
	// x86.
	// Assuming the same case for NSX
#ifdef GOARCH_386
